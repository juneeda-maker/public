	...
	<div class="container">
		<div class="row">
			<table class="table table-striped" style="text-align: center; border: 1px solid #dddddd;">
				...
				<tbody>
					...
					<tr>
						...
						<td><a href = "debateview.jsp?bbsID=<%=list.get(i).getBbsID()%>"><%=list.get(i).getBbsTitle() %></a></td>
			...
			<%
				if(pageNumber !=1){
			%>
				<a href="debate.jsp?pageNumber=<%=pageNumber - 1%>"class="btn btn-sucess btn-arrow-left">이전</a> 
			<%	//게시글이 10개 이상 작성 됐을시 이전버튼 생성
				}if(debateDAO.nextPage(pageNumber + 1)){
			%>	
			<a href="debate.jsp?pageNumber=<%=pageNumber +1%>"class="btn btn-sucess btn-arrow-left">다음</a>
			<%
				}
			%>
			<a href="debatewrite.jsp" class="btn btn-primary pull-right">글쓰기</a>
		</div>
	</div>
